...
def parse_out_file(out_filepath):
    ...
    주어진 .out 파일에서 다음 형식만 추출합니다:
      ...
      - "     PWSCF        :      1h 2m CPU      1h 3m WALL"
    """
    total_energy = None
    cpu_time = None

    energy_pattern = re.compile(r"!\s+total energy\s*=\s*([-+]?\d*\.?\d+)\s*Ry")
    pwscf_pattern = re.compile(r"PWSCF\s*:\s*(?:(\d+)h)?\s*(?:(\d+)m)?\s*CPU")
    ...
    with open(out_filepath, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if cpu_time is None:
                ...
                if pwscf_match:
                    try:
                        hours = int(pwscf_match.group(1)) if pwscf_match.group(1) else 0
                        minutes = int(pwscf_match.group(2)) if pwscf_match.group(2) else 0
                        cpu_time = hours * 3600 + minutes * 60
                    except ValueError:
                        pass
...
def main():
    ...
    ax1.set_xlabel(f"{sweep_key} 값")
    ax1.set_ylabel("Total Energy (Ry)", color=color_energy)
    ax1.plot(sweep_values, energies, color=color_energy, marker="o", label="Total Energy")
